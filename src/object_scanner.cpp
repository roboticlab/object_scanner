...
void ObjectScanner::integrateCloud(pcl::PointCloud<pcl::PointXYZRGB> cloud, Eigen::Affine3f trans)
{
	_tsdf->integrateCloud(trans, cloud);
}
pcl::PointCloud< pcl::PointXYZRGB >::Ptr ObjectScanner::getCloud(Eigen::Affine3f trans)
{
...
void ObjectScanner::TSDFtest()
{
	...
	integrateCloud(*cloud0, Eigen::Affine3f::Identity());
	...
	integrateCloud(*cloud25, Eigen::Affine3f(Eigen::Translation3f(Eigen::Vector3f(0.02,0.0,0.0))));
	...
	integrateCloud(*cloud50, Eigen::Affine3f(Eigen::Translation3f(Eigen::Vector3f(0.04,0.0,0.0))));
	
	pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud25_transformed (new pcl::PointCloud<pcl::PointXYZRGB>);
	pcl::transformPointCloud (*cloud50, *cloud25_transformed, Eigen::Affine3f(Eigen::Translation3f(Eigen::Vector3f(0.02,0.0,0.0))));
	
	pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud50_transformed (new pcl::PointCloud<pcl::PointXYZRGB>);
	pcl::transformPointCloud (*cloud50, *cloud50_transformed, Eigen::Affine3f(Eigen::Translation3f(Eigen::Vector3f(0.04,0.0,0.0))));
	
	pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud_ = _tsdf->getCloud(Eigen::Affine3f::Identity());
